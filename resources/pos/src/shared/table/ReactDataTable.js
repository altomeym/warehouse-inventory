...
console.log('data type productCatOptions ', productCatOptions)
    ...
    const onAdjustmentChange = (obj) => {
        dispatch({type: 'RESET_OPTION', payload: false})
        setTableAdjustmentValue(obj);
        dispatch({type: 'ON_TOGGLE', payload: false});
    };

    const onProductBrandOptionChange = (obj) => {
        dispatch({type: 'RESET_OPTION', payload: false})
        setTableBrandProductValue(obj);
        dispatch({type: 'ON_TOGGLE', payload: false});
    };
    ...
    const subHeaderComponentMemo = React.useMemo(() => {
        return (
            <>
                ...
                <Col xxl={isShowSearch ? 12 : 8} className='d-flex flex-wrap align-items-center justify-content-end col-12 col-md-8 col-lg-8'>
                    ...
                    {isExport ?
                        <div className='text-end'>
                            <Button type='button' variant='primary' onClick={() => onExcelClick()}
                                    className='me-3 btn-light-primary'> {getFormattedMessage("product.export.title")}</Button>
                        </div> : ''}
                    ...
                    <div className='text-end order-2'>
                        <Button variant="primary" className='me-3 my-2 btn-light-primary' onClick={goToImportProduct}>
