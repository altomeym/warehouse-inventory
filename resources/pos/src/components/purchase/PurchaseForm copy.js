...
const PurchaseForm = (props) => {
    ...
    let removeTaxFormFields = (i) => {
        ...
        newFormValues.splice(i, 1);
        setCustomTaxDynamicFields(newFormValues)
        calTaxTotal(customTaxDynamicFields[i]['tax_value'])
       
    }
    let calTaxTotal = (singleVal=0)=>{
        let totalTax = 0;
        if(singleVal)
          totalTax =  parseFloat(purchaseValue.tax_rate) -   parseFloat(singleVal);
        else
      ...
      setPurchaseValue(inputs => ({...inputs, ['tax_rate']: totalTax && totalTax}))
    }
