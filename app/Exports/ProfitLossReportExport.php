...
class ProfitLossReportExport implements FromView
{
    public function view(): \Illuminate\Contracts\View\View
    {
        ...
        if(!empty($startDate) && !empty($endDate))
        {
                ...
                $data['sales'] = Sale::sum('grand_total');
                $data['purchases'] = Purchase::sum('grand_total');
                $data['sale_returns'] = SaleReturn::sum('grand_total');
                $data['purchase_returns'] = PurchaseReturn::sum('grand_total');
                $data['expenses'] = Expense::sum('amount');
                $data['sales_payment_amount'] = SalesPayment::sum('amount');
                $data['Revenue'] = $data['sales'] - $data['sale_returns'];
                $data['payments_received'] = $data['sales_payment_amount'] + $data['purchase_returns'];
